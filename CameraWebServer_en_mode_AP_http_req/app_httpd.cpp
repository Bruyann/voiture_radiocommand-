...
#define LED_LEDC_GPIO            4//22  //configure LED pin
...
*/
  ...
  // Ajouter les routes pour gérer le flash LED
    httpd_uri_t flash_on_uri = {
        .uri = "/flash/on",
        .method = HTTP_GET,
        .handler = [](httpd_req_t *req) {
            digitalWrite(4, 1); // Allumer le flash (GPIO 4 pour AI-Thinker)
            httpd_resp_send(req, "LED allumée", HTTPD_RESP_USE_STRLEN);
            return ESP_OK;
        },
        .user_ctx = NULL
    };
    ...
    httpd_uri_t flash_off_uri = {
        ...
        .method = HTTP_GET,
        .handler = [](httpd_req_t *req) {
            digitalWrite(4, 0); // Éteindre le flash
            httpd_resp_send(req, "LED éteinte", HTTPD_RESP_USE_STRLEN);
            return ESP_OK;
        },
        .user_ctx = NULL
    };
  // Ajouter les routes pour gérer le moteur
    httpd_uri_t avant_on_uri = {
        .uri = "/avant",
        .method = HTTP_GET,
        .handler = [](httpd_req_t *req) {
  ...
  // Ajouter les routes pour gérer le moteur
    httpd_uri_t gauche_on_uri = {
        ...
        .handler = [](httpd_req_t *req) {
            ...
            digitalWrite(led3, LOW);
            digitalWrite(led4, LOW);
            httpd_resp_send(req, "Moteur à gauche", HTTPD_RESP_USE_STRLEN);
            return ESP_OK;
        },
        .user_ctx = NULL
    };
  ...
  // Ajouter les routes pour gérer le moteur
    httpd_uri_t droite_on_uri = {
        ...
        .handler = [](httpd_req_t *req) {
            ...
            digitalWrite(led3, HIGH);
            digitalWrite(led4, LOW);
            httpd_resp_send(req, "Moteur à droite", HTTPD_RESP_USE_STRLEN);
            return ESP_OK;
        },
        .user_ctx = NULL
    };

    httpd_uri_t stop_on_uri = {
        .uri = "/stop",
        .method = HTTP_GET,
        .handler = [](httpd_req_t *req) {
            digitalWrite(led1, LOW);
            ...
            digitalWrite(led3, LOW);
            digitalWrite(led4, LOW);
            httpd_resp_send(req, "Moteur arrêté", HTTPD_RESP_USE_STRLEN);
            return ESP_OK;
        },
        .user_ctx = NULL
    };
  ...
  ra_filter_init(&ra_filter, 20);
    ...
    if (httpd_start(&camera_httpd, &config) == ESP_OK) {
      ...
      httpd_register_uri_handler(camera_httpd, &avant_on_uri);
      httpd_register_uri_handler(camera_httpd, &arriere_on_uri);
      httpd_register_uri_handler(camera_httpd, &droite_on_uri);
      //httpd_register_uri_handler(camera_httpd, &gauche_on_uri);
      //httpd_register_uri_handler(camera_httpd, &stop_on_uri);

    }
    if (httpd_register_uri_handler(camera_httpd, &gauche_on_uri) == ESP_OK) {
        Serial.println("Route /gauche enregistrée avec succès !");
    } else {
        Serial.println("Erreur : Impossible d’enregistrer /gauche");
    }

    if (httpd_register_uri_handler(camera_httpd, &stop_on_uri) == ESP_OK) {
        Serial.println("Route /stop enregistrée avec succès !");
    } else {
        Serial.println("Erreur : Impossible d’enregistrer /stop");
    }
    ...
    config.ctrl_port += 1;
    log_i("Routes moteur enregistrées: /moteur/avant, /moteur/arriere, /moteur/droite, /moteur/gauche, /moteur/stop");
