...
void startCameraServer() {
  ...
  // Ajouter les routes pour gérer le flash LED
    ...
    //------------------------- MOTEUR
    httpd_uri_t moteur_avant_uri = {
        .uri = "/moteur/avant",
        .method = HTTP_GET,
        .handler = [](httpd_req_t *req) {
            digitalWrite(4, 0); // Éteindre le flash
            //moteur Av
            digitalWrite(12, 0); // Activer direction 1
            digitalWrite(15, 0); // Activer direction 1
            digitalWrite(13, 1);
            digitalWrite(14, 1);
            httpd_resp_send(req, "moteur avant", HTTPD_RESP_USE_STRLEN);
            delay(2000);
            ...
            digitalWrite(13, 0);
            digitalWrite(14, 0);
            httpd_resp_send(req, "moteur stop", HTTPD_RESP_USE_STRLEN);
            return ESP_OK;
        },
        .user_ctx = NULL
    };

        httpd_uri_t moteur_arriere_uri = {
        .uri = "/moteur/arriere",
        .method = HTTP_GET,
        .handler = [](httpd_req_t *req) {
            digitalWrite(4, 1); // Allumer le flash (GPIO 4 pour AI-Thinker)
            //moteur Ar
            digitalWrite(12, 1); // Activer direction 1
            digitalWrite(15, 1); // Activer direction 1
            digitalWrite(13, 0);
            digitalWrite(14, 0);
            httpd_resp_send(req, "moteur arriere", HTTPD_RESP_USE_STRLEN);
            delay(2000);
  ...
  ra_filter_init(&ra_filter, 20);
    ...
    if (httpd_start(&camera_httpd, &config) == ESP_OK) {
      ...
      //moteur
      httpd_register_uri_handler(camera_httpd, &moteur_avant_uri);
      httpd_register_uri_handler(camera_httpd, &moteur_arriere_uri);
